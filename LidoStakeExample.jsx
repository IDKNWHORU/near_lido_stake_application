// define functions
const getSender = () => {
  return isSenderUndefined() === true
    ? ""
    : `${state.sender.substring(0, 6)}...${state.sender.substr(-4)}`;
};

const isSenderUndefined = () => {
  return state.sender === undefined;
};

const isThemeNotMounted = () => {
  return state.theme === undefined || state.theme === null;
};
...
const setAccount = () => {
  ...
  if (accounts.length > 0) {
    ...
    console.log("set sender", accounts);
  }
};
...
const isLidoArpUndefined = () => {
  return state.lidoArp === undefined;
};
...
const setArp = () => {
  ...
  if (!arp) return;
  State.update({ lidoArp: JSON.parse(arp?.body?.contents) ?? "..." });
};
...
const isBalnaceUndefined = () => {
  return state.balance === undefined;
};
...
const isStakedBalanceUndefined = () => {
  return state.stakedBalance === undefined;
};

const setStakedBalance = () => {
  getStakedBalance(state.sender).then((stakedBalance) => {
    State.update({ stakedBalance });
  });
};

const isTxCostUndefined = () => {
  return state.txCost === undefined;
};
...
const isChainIdUndefined = () => {
  return state.chainId === undefined;
};

const isEhtersUndefined = () => {
  return ethers === undefined;
};
...
// on after load application

if (
  isChainIdUndefined() === true &&
  isEhtersUndefined() === false &&
...
const cssFont = fetch(props.fontLink).body;
const css = fetch(props.cssLink).body;

if (!cssFont || !css) return "";

if (isThemeNotMounted() === true) {
  setTheme();
}

if (isSenderUndefined() === true) {
  setAccount();
}

if (isBalnaceUndefined() === true && isSenderUndefined() === false) {
  setBalance();
}

if ((isStakedBalanceUndefined() === true && isSenderUndefined()) === false) {
  setStakedBalance();
}

if (isTxCostUndefined() === true) {
  setTxCost();
}

if (isLidoArpUndefined() === true) {
  setArp();
}

// relate to user view components

const Theme = state.theme;

const LidoStakeApplication = () => (
  <Theme>
...
return (
  <>
    {isSenderUndefined() === false ? <LidoStakeApplication /> : <Web3Connect />}
  </>
);
